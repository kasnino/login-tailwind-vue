<template>
  <div class="divide-y divide-slate-100 bg-gray-900 ">

    <List>
      <ListItem class="hover:text-white" v-for="movie in movies" :key="movie.id" :movie="movie" />
    </List>
  </div>
</template>

<script>
  import Nav from './Lista/Nav.vue'
  import NavItem from './Lista/NavItem.vue'
  import List from './Lista/List.vue'
  import ListItem from './Lista/ListItem.vue'

  export default {
    props: ['movies'],
    components: {
      Nav,
      NavItem,
      List,
      ListItem,
    },

    data(){
        return{
        movies:[{
        title: "Dawn of the Planet of the Apes",
        image: "http://api.androidhive.info/json/movies/1.jpg",
        rating: 8.3,
        "runtime": 2014,
        "genre": ["Action", "Drama", "Sci-Fi"]
    },
    {
        title: "District 9",
        image: "https://api.androidhive.info/json/movies/2.jpg",
        rating: 8,
        "runtime": 2009,
        "genre": ["Action", "Sci-Fi", "Thriller"]
    },
    {
        title: "Transformers: Age of Extinction",
        image: "http://api.androidhive.info/json/movies/3.jpg",
        rating: 6.3,
        "runtime": 2014,
        "genre": ["Action", "Adventure", "Sci-Fi"]
    },
    {
        title: "X-Men: Days of Future Past",
        image: "http://api.androidhive.info/json/movies/4.jpg",
        rating: 8.4,
        "runtime": 2014,
        "genre": ["Action", "Sci-Fi", "Thriller"]
    },
    {
        title: "The Machinist",
        image: "http://api.androidhive.info/json/movies/5.jpg",
        rating: 7.8,
        "runtime": 2004,
        "genre": ["Drama", "Thriller"]
    },
    {
        title: "The Last Samurai",
        image: "http://api.androidhive.info/json/movies/6.jpg",
        rating: 7.7,
        "runtime": 2003,
        "genre": ["Action", "Drama", "History"]
    },
    {
        title: "The Amazing Spider-Man 2",
        image: "http://api.androidhive.info/json/movies/7.jpg",
        rating: 7.3,
        "runtime": 2014,
        "genre": ["Action", "Adventure", "Fantasy"]
    },
    {
        title: "Tangled",
        image: "http://api.androidhive.info/json/movies/8.jpg",
        rating: 7.9,
        "runtime": 2010,
        "genre": ["Action", "Drama", "Sci-Fi"]
    },
    {
        title: "Rush",
        image: "http://api.androidhive.info/json/movies/9.jpg",
        rating: 8.3,
        "runtime": 2013,
        "genre": ["Animation", "Comedy", "Family"]
    },
    {
        title: "Drag Me to Hell",
        image: "http://api.androidhive.info/json/movies/10.jpg",
        rating: 6.7,
        "runtime": 2009,
        "genre": ["Horror", "Thriller"]
    },
    {
        title: "Despicable Me 2",
        image: "http://api.androidhive.info/json/movies/11.jpg",
        rating: 7.6,
        "runtime": 2013,
        "genre": ["Animation", "Comedy", "Family"]
    },
    {
        title: "Kill Bill: Vol. 1",
        image: "http://api.androidhive.info/json/movies/12.jpg",
        rating: 8.2,
        "runtime": 2003,
        "genre": ["Action", "Crime"]
    },
    {
        title: "A Bug's Life",
        image: "http://api.androidhive.info/json/movies/13.jpg",
        rating: 7.2,
        "runtime": 1998,
        "genre": ["Animation", "Adventure", "Comedy"]
    },
    {
        title: "Life of Brian",
        image: "http://api.androidhive.info/json/movies/14.jpg",
        rating: 8.9,
        "runtime": 1972,
        "genre": ["Comedy"]
    },
    {
        title: "How to Train Your Dragon",
        image: "http://api.androidhive.info/json/movies/15.jpg",
        rating: 8.2,
        "runtime": 2010,
        "genre": ["Animation", "Adventure", "Family"]
    }]
        }
    }
  }
</script>